...
function cartoonVideoProcessor(
  videoPath,
  outputDir,
  introDir,
  assetsDir,
  ...
  const OVERLAY_DURATION = 1.8;
  const OVERLAY_MIN_GAP = 6;
  const OVERLAY_MAX_GAP = 15;
  const BLUR_STRENGTH = 3;
  const VOICE_PITCH = 0.82;
  const ORIGINAL_AUDIO_VOLUME = 0.7;
  const BED_AUDIO_VOLUME = 0.35;
  ...
  const cartoonIntensity = 0.3; // Cartoon effect strength - reduced for better quality
  ...
  function buildOverlayPlan(mainDuration, assetCount) {
    ...
    const MAX_OVERLAYS = 120; // More overlays for copyright avoidance

    while (overlays.length < MAX_OVERLAYS) {
      const gap =
        OVERLAY_MIN_GAP + Math.random() * (OVERLAY_MAX_GAP - OVERLAY_MIN_GAP);
      current += gap;
      if (current + OVERLAY_DURATION > dur - 2) break;
      overlays.push({
  ...
  async function processVideo() {
    ...
    fs.mkdirSync(tempPartDir, { recursive: true });
    ...
    const outputFileName =
      `${fileBaseName}`
        ...
        .replace(/[^a-zA-Z0-9-]/g, "-")
        .replace(/-+/g, "-")
        .toLowerCase() + `-cartoon-${randomId}-by-nhrepon.mp4`;
    ...
    if (!fs.existsSync(introVideo))
      throw new Error(`intro.mp4 not found at ${introDir}`);
    ...
    console.log(`🎨 Cartoon Processing Started!`);
    ...
    // CARTOON EFFECTS - Balanced for better visual quality
    const cartoonFilter1 = `colorbalance=rs=0.3:gs=0.3:bs=0.3`; // Subtle color shift
    const cartoonFilter2 = `eq=contrast=1.15:brightness=0.02:saturation=1.2`; // Enhanced colors
    const cartoonFilter3 = `scale=1440:1080:flags=neighbor,scale=${OUTPUT_SIZE}:${OUTPUT_SIZE}:flags=neighbor`; // Mild pixelation
    const cartoonFilter4 = `unsharp=5:5:1.0:3:3:0.3`; // Light sharpening
    const noiseReductionFilter = "hqdn3d=2:1:3:2"; // Light noise reduction

    // Combined cartoon effect
    const combinedCartoonFilter = `${cartoonFilter1},${cartoonFilter2},${cartoonFilter3},${cartoonFilter4},${noiseReductionFilter}`;

    // Main video with cartoon effects
    fc.push(
      `[0:v]setpts=PTS/${SPEED_FACTOR},${combinedCartoonFilter},split=2[mainA][mainB]`,
    );
    fc.push(
      `[mainA]scale=${HALF_SIZE}:${HALF_SIZE}:force_original_aspect_ratio=increase,crop=${HALF_SIZE}:${HALF_SIZE},boxblur=${BLUR_STRENGTH},scale=${OUTPUT_SIZE}:${OUTPUT_SIZE},eq=brightness=0.02:contrast=1.08[mainbg]`,
    );
    fc.push(
      `[mainB]hflip,scale=${OUTPUT_SIZE}:${OUTPUT_SIZE}:force_original_aspect_ratio=increase,crop=${OUTPUT_SIZE}:${OUTPUT_SIZE}:(iw-ow)/2:(ih-oh)/2,eq=brightness=0.03:contrast=1.12[mainfg]`,
    );
    ...
    overlays.forEach((ov, idx) => {
      ...
      fc.push(
        `[${overallInputIndex}:v]scale=${OUTPUT_SIZE}:${OUTPUT_SIZE}:force_original_aspect_ratio=increase,crop=${OUTPUT_SIZE}:${OUTPUT_SIZE},format=rgba,colorchannelmixer=aa=${OVERLAY_OPACITY},eq=saturation=1.2${overlayLabel}`,
      );
    ...
    if (showTopText) {
      fc.push(
        `${currentMainLabel}drawtext=text='${brandText}':fontfile='${drawtextFont}':fontcolor=white:fontsize=42:borderw=3:bordercolor=black:x=40:y=20:fix_bounds=true:enable='gte(t,0)'[maintoptext]`,
      );
    }
    ...
    // Audio with cartoon-style processing
    fc.push(
      `[0:a]asetrate=44100*${VOICE_PITCH},aresample=44100,highpass=f=100,lowpass=f=6000,afftdn=nf=-20,${atempoFilters(SPEED_FACTOR / VOICE_PITCH)},volume=${ORIGINAL_AUDIO_VOLUME}[mainorig]`,
    );
    ...
    const outputOptions = [
      ...
      "-1",
      "-metadata",
      `title=${fileBaseName} | cartoon video`,
      "-metadata",
      "description=Cartoon-style video clip featuring animated scenes and artistic effects created for social media and short-form content.",
      "-metadata",
      "comment=Cartoon processing by NHRepon",
      "-metadata",
      ...
      "language=en",
      "-metadata",
      "encoder=Lavf/NHRepon Cartoon Processor",
      "-metadata",
      "composer=NHRepon",
      "-metadata",
      "album=Cartoon Animation Collection",
      "-metadata",
      "track=1",
      "-metadata",
      "network=NHRepon",
      "-metadata",
      "synopsis=Cartoon-style animated video clip created with artistic effects for social sharing and entertainment.",
      "-metadata",
      `keywords=${[
        "cartoon",
        "animation",
...
const partDir = path.join(__dirname, "output/cartoonParts");
...
async function run() {
  ...
  for (const inputVideo of inputFiles) {
    const results = await cartoonVideoProcessor(
      inputVideo,
      outputDir,
      introDir,
      assetsDir,
    ...
    console.log(`🎨 Finished cartoon processing for ${inputVideo}`);
    console.log(`✅ Total cartoon videos processed: ${results.length}`);
  }
}

module.exports = {
  cartoonVideoProcessor,
  run,
};
