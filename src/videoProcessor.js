...
const projectRoot = path.join(__dirname, ".."); // adjust if you run from repo root
...
async function processVideo() {
  ...
  fc.push(
    `[mainB]hflip,scale=1400:1400:force_original_aspect_ratio=decrease,${gradeFilter}[mainfg]`,
  );
  ...
  fc.push(
    `${currentMainLabel}drawtext=text='${brandText}':fontfile='${drawtextFont}':fontcolor=white:fontsize=28:borderw=2:bordercolor=black:box=1:boxcolor=black@0.55:boxborderw=18:x=(w-text_w)/2:y=60:fix_bounds=true:enable='gte(t,0)'[maintoptext]`,
  );
  fc.push(
    `[maintoptext]drawtext=text='${brandText}':fontfile='${drawtextFont}':fontcolor=white:fontsize=28:borderw=2:bordercolor=black:box=1:boxcolor=black@0.55:boxborderw=18:x=(w-text_w)/2:y=h-text_h-60:fix_bounds=true:enable='gte(t,0)',setsar=1[mainv]`,
  );
  ...
  command
    ...
    .on("end", async () => {
      try {
        ...
        console.log("Completed but failed to hash output:", err.message || err);
      }
    })
